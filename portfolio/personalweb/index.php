<?php 
$url=$_SERVER['REQUEST_URI'];

       $title='Pt. Prashant Triapthi :: Personal Website ';
       $nav=3;
       include '../../header.php';
?>

    <div class="container">

        <div class="row">

            <div class="col-xs-12 ">
                <div class="panel panel-primary">
                    <div class="panel-heading panel-heading-custom" style="background: #F44336; color: #fff; font-size:20px;">Prashanttripathi.ga <a href="/ portfolio/" style="color:#fff"><span class="pull-right hidden-xs">View more projects</span></a></div>
                    <div class="panel-body" style="padding:50px;">
                        <div class="row">
                            <div class="col-xs-12 col-sm-6">
                                <p class="title1">Prashanttripathi.ga</p>
                                <p>It is a Personal Website.It is project of my web development </p>
                                <p>It is Fully responsive and dyanmic .</p>
								<p>To visit the website. Click here <a href="https://prashanttripathi.ga" style="color:blue;" target="_blank">https://prashanttripathi.ga</a></p>

                            </div>

                            <div class="col-xs-12 col-sm-6">
                                <img src="/image/personalweb.jpg" class="img img-thumbnail img-responsive" style="height:200px;width:300px">
                            </div>

                        </div>
                        <br>
                        <br>

                        <div class="row">
                            <div class="col-xs-12 col-sm-6">
                                <p class="title1">Features</p>
                                <hr width="30%" style="margin:0px">
                                <br>
                                <!--quick fact start-->
                                <p>✔&nbsp;Personal Portpolio.</p>
                                <p>✔&nbsp;About Me.</p>
                                <p>✔&nbsp;Portfolio.</p>
                                <p>✔&nbsp;Career timeline.</p>
                                <p>✔&nbsp;Achievements.</p>
                                <p>✔&nbsp;Contact Details.</p>
                                <p>✔&nbsp;Detailed info of all project.</p>
								<p>✔&nbsp;Responsive mobile freindly.</p>
                            </div>

                            <div class="col-xs-12 col-sm-6">
                                <p class="title1">Technology use</p>
                                <hr width="30%" style="margin:0px">
                                <br>
                                <!--quick fact start-->
                                <p>✔&nbsp;Use HTML,CSS3 and bootstrap for designing.</p>
                                <p>✔&nbsp;For designing use of CSS3,Font-awesome icons.</p>
                                <p>✔&nbsp;<b style="color:#F44336">Custom Content Management system</b> using PHP and MYSQL.</p>
                                <p>✔&nbsp;PHP for backend.</p>
                                <p>✔&nbsp;Javascript for dyanmic behaviour.</p>
                                <p>✔&nbsp;Git for version control.</p>
								<p>✔&nbsp;Apache server for hosting.</p>
                            </div>

                        </div>

                    </div>
                </div>
            </div>

        </div>
    </div>

    <?php include '../../footer.php' ;?>